...
11. **Security First**: When constructing database queries or filters (especially for PocketBase), always use the provided security utilities (e.g., `src.core.security.sanitize_pb_filter`) instead of manual string manipulation. Verify input sanitization with test cases containing special characters (e.g., backslashes and quotes).
