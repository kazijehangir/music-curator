...
def cleanup_orphaned_releases() -> Dict[str, Any]:
    ...
    # ⚡ Bolt: Parallelize independent database deletions to eliminate N+1 network roundtrips.
    # Impact: Reduces deletion time from O(N) sequential HTTP calls to O(N/Workers), significantly speeding up large cleanup operations.
    import concurrent.futures

    def _delete_release(rid: str) -> tuple[bool, str]:
        try:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(_delete_release, rid): rid for rid in orphan_ids}
