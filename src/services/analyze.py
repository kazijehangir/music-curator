...
def run_analysis(pb: Optional[Any] = None) -> Dict[str, Any]:
    ...
    for i, record in enumerate(unanalyzed_records):
        ...
        try:
            ...
            safe_record_id = sanitize_pb_filter(record.id)
            duplicate_files = pb.collection(COLL_FILE).get_list(
                1, 2, {"filter": f"{MusicFile.ACOUSTID_FP}='{safe_fp}' && id!='{safe_record_id}'"}
            )
            ...
            safe_target_release_id = sanitize_pb_filter(target_release_id)
            siblings = pb.collection(COLL_FILE).get_full_list(
                query_params={"filter": f"{MusicFile.RELEASE}='{safe_target_release_id}'", "sort": f"-{MusicFile.QUALITY_SCORE}"}
            )
