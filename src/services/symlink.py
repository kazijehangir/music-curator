...
def run_symlink() -> Dict[str, Any]:
    ...
    # 2. Extract needed release IDs and fetch them in chunks to avoid Fetch All
    release_ids = list({getattr(f, MusicFile.RELEASE) for f in primary_files if getattr(f, MusicFile.RELEASE, None)})
    releases_by_id = {}
