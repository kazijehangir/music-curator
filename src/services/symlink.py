...
def run_symlink() -> Dict[str, Any]:
    ...
    # ⚡ Bolt Optimization: Use the 'fields' parameter to fetch only required columns.
    # Expected Impact: Significantly reduces network payload size and memory footprint.
    all_releases = pb.collection(COLL_RELEASE).get_full_list(
        query_params={"fields": f"id,{Release.ARTIST},{Release.TITLE},{Release.ALBUM}"}
    )
    releases_by_id = {r.id: r for r in all_releases}

    # 2. Fetch all primary files
    # ⚡ Bolt Optimization: Restrict queried fields to prevent memory bloat as the library grows.
    ...
    # ⚡ Bolt Optimization: Restrict queried fields.
