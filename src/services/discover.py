...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    for dir_name in ingest_folders:
        ...
        # Pre-fetch all files for this source_dir to avoid N+1 queries
        existing_records_map = {}
        try:
            # We fetch all fields to match previous behavior exactly.
            # get_full_list can be memory intensive for huge collections, but filtering by source_dir mitigates this.
            records = pb.collection('music_file').get_full_list(
                query_params={"filter": f"source_dir='{dir_name}'"}
            )
            for r in records:
                path_val = getattr(r, 'file_path', None)
                if path_val:
                    existing_records_map[path_val] = r
        except Exception as e:
            print(f"DEBUG: Failed to pre-fetch records for {dir_name}: {e}")
            errors.append(f"Failed to pre-fetch records for {dir_name}: {e}")
            # If pre-fetch fails, we should probably skip this folder to avoid mass-duplicate inserts
            continue
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    # Check if file exists in PocketBase via pre-fetched map
                    file_path_str = str(filepath)

                    if file_path_str in existing_records_map:
                        # File exists — check if size/mtime changed
                        existing_record = existing_records_map[file_path_str]
