...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    # Pre-fetch existing files for O(1) lookup to fix N+1 query problem
    try:
        all_existing = pb.collection('music_file').get_full_list(
            query_params={"fields": "id,file_path,file_hash"}
        )
        existing_by_path = {
            getattr(r, 'file_path', ''): r for r in all_existing
        }
    except Exception as e:
        print(f"Error pre-fetching existing files: {e}")
        return {
            "status": "error",
            "new_files": 0,
            "updated_files": 0,
            "errors": [f"Failed to fetch existing files from PocketBase: {e}"]
        }
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    if file_path_str in existing_by_path:
                        # File exists — check if size/mtime changed
                        existing_record = existing_by_path[file_path_str]
