...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    # Bulk fetch existing files to prevent N+1 queries in the loop
    existing_files = {}
    try:
        ...
        for record in all_records:
            path_val = getattr(record, 'file_path', None)
            if path_val:
                existing_files[path_val] = record
    except Exception as e:
        errors.append(f"Failed to bulk fetch existing files: {e}")
        return {
            "status": "error",
