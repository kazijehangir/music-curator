...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    for dir_name in ingest_folders:
        ...
        # Pre-fetch existing records for this source_dir to avoid N+1 queries in the loop
        ...
        try:
            ...
            for record in existing_records:
                # Store by file_path for fast O(1) lookups
                file_path = getattr(record, 'file_path', None)
                if file_path:
                    existing_records_dict[file_path] = record
        except Exception as e:
            print(f"DEBUG: Failed to pre-fetch records for {dir_name}: {e}")
            errors.append(f"Error pre-fetching for {dir_name}: {str(e)}")
            continue
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    # Check if file exists in PocketBase via in-memory dict
