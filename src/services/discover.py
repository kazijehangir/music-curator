...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    # Bulk prefetch all existing records to avoid N+1 queries in the loop
    try:
        all_records = pb.collection('music_file').get_full_list()
        existing_records = {getattr(r, 'file_path', ''): r for r in all_records}
    except Exception as e:
        print(f"ERROR: Failed to prefetch existing records: {e}")
        raise e
