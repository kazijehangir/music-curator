...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    for dir_name in ingest_folders:
        ...
        # ⚡ Bolt: Fetch all existing files for this directory in one query (N+1 optimization)
        try:
            safe_dir_name = dir_name.replace("'", "\\'")
            # We don't use fields parameter to avoid omitting fields used downstream
            existing_records = pb.collection('music_file').get_full_list(
                query_params={"filter": f"source_dir='{safe_dir_name}'"}
            )
            existing_files = {getattr(r, 'file_path', ''): r for r in existing_records}
        except Exception as e:
            print(f"STATUS: Error fetching existing records for {dir_name}: {e}")
            errors.append(f"Database error fetching existing files for {dir_name}: {e}")
            continue
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    # Check if file exists in PocketBase using in-memory dict
                    file_path_str = str(filepath)

                    if file_path_str in existing_files:
                        # File exists — check if size/mtime changed
                        existing_record = existing_files[file_path_str]
