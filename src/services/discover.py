...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    for dir_name in ingest_folders:
        ...
        # Pre-fetch all files for this directory to avoid N+1 query problem
        existing_records_dict = {}
        try:
            # We don't use fields='id,file_path,file_hash' to be fully safe with downstream
            records = pb.collection('music_file').get_full_list(
                query_params={"filter": f"source_dir='{dir_name}'"}
            )
            existing_records_dict = {getattr(r, 'file_path', ''): r for r in records}
        except Exception as e:
            print(f"ERROR: Failed to fetch existing records for {dir_name}: {e}")
            errors.append(f"Failed to fetch existing records for {dir_name}: {e}")
            continue
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    # Check if file exists in PocketBase (in-memory lookup)
