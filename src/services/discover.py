...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    print("STATUS: Pre-fetching existing files from PocketBase...")
    try:
        all_existing_files = pb.collection('music_file').get_full_list()
    except Exception as e:
        errors.append(f"Failed to pre-fetch files: {e}")
        return {
            "status": "error",
            "new_files": 0,
            "updated_files": 0,
    ...
    existing_files_map = {getattr(record, 'file_path', ''): record for record in all_existing_files}
