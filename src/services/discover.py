...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    # Optimization: Pre-fetch all file paths and hashes to avoid N+1 queries.
    existing_files_map = {}
    try:
        # get_full_list returns a standard list of Record objects (no .items wrapper)
        ...
        for r in all_records:
            ...
            if path_val:
                existing_files_map[path_val] = r
    except Exception as e:
        print(f"WARNING: Failed to pre-fetch existing files: {e}")
        errors.append(f"Failed to pre-fetch existing files: {e}")
