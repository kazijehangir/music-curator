...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        existing_files_cache = {getattr(rec, 'file_path'): rec for rec in all_records if getattr(rec, 'file_path')}
    except Exception as e:
        return {
            "status": "error",
            "new_files": 0,
            "updated_files": 0,
            "errors": [f"Failed to fetch existing files: {e}"]
        }
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    if file_path_str in existing_files_cache:
                        # File exists — check if size/mtime changed
                        existing_record = existing_files_cache[file_path_str]
