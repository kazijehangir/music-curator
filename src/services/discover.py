...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    for dir_name in ingest_folders:
        ...
        # Pre-fetch all existing records for this source_dir to avoid N+1 query problem
        # Query fails fast on purpose if DB is unavailable, preventing duplicate creations.
        ...
        try:
            ...
            # Create an in-memory dictionary for O(1) lookups by file_path
            existing_records = {getattr(r, 'file_path'): r for r in existing_records_list}
        except Exception as e:
            errors.append(f"Failed to fetch existing records for directory {dir_name}: {e}")
            continue
        ...
        for root, _, files in os.walk(ingest_path):
            for file in files:
                ...
                try:
                    ...
                    # Check if file exists in PocketBase (O(1) dictionary lookup)
                    file_path_str = str(filepath)
                    existing_record = existing_records.get(file_path_str)
