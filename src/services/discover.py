...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    try:
        print("STATUS: Bulk fetching existing files from PocketBase to prevent N+1 queries...")
        all_files = pb.collection('music_file').get_full_list(
            query_params={"fields": "id,file_path,file_hash"}
        )
        existing_files_map = {getattr(r, 'file_path'): r for r in all_files if getattr(r, 'file_path', None)}
    except Exception as e:
        msg = f"Failed to bulk fetch existing files: {e}"
        print(f"ERROR: {msg}")
        errors.append(msg)
        return {
            "status": "error",
