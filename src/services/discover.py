...
def run_discovery(pb: Optional[PocketBase] = None, ingest_folders: Optional[list[str]] = None) -> Dict[str, Any]:
    ...
    # Pre-fetch all existing file records to avoid N+1 queries during discovery
    existing_files_dict = {}
    try:
        ...
        for record in all_existing_files:
            if hasattr(record, 'file_path') and record.file_path:
                existing_files_dict[record.file_path] = record
    except Exception as e:
        print(f"ERROR: Failed to pre-fetch existing files from PocketBase: {e}")
        errors.append(f"Database error during pre-fetch: {e}")
        return {"status": "error", "new_files": 0, "updated_files": 0, "errors": errors}
