...
def run_tagging(pb: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        # Build filter to only fetch files for the releases we are about to process
        # We chunk the IDs to avoid exceeding SQLite/HTTP limits if there are many releases.
        files_by_release = {}
        ...
        logger.error(f"Failed to prefetch files for tagging: {e}")
    ...
    for idx, r in enumerate(releases):
        ...
        try:
            ...
            success = process_release(pb, r, release_files, stats)
            if success:
                stats["tagged"] += 1
        except Exception as e:
