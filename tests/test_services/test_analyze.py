...
def test_cleanup_handles_delete_error(mocker):
    ...
    # In concurrent map, side_effects mapped to threaded calls can happen out of order.
    # Let's target exactly rel1 failing.
    def mock_delete(rid):
        if rid == "rel1":
    ...
    assert any("rel1" in e for e in result["errors"])
