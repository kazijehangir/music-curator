...
def test_run_discovery_update_file(tmp_path, mocker):
    ...
    existing_record = mocker.MagicMock(file_path=str(yubal_dir.joinpath("existing_song.flac")), file_hash="old_hash", id="rec_123")
...
def test_run_discovery_prefetch_failure(mocker):
    mock_pb_client = mocker.MagicMock()
    mocker.patch("src.services.discover.get_pb_client", return_value=mock_pb_client)
    ...
    assert "Failed to prefetch existing files: DB offline" in result["errors"][0]
