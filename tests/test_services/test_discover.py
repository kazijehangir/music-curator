...
def test_run_discovery_update_file(tmp_path, mocker):
    ...
    # Mock existing record with a DIFFERENT hash and correct path
    expected_path = str(tmp_path / "downloads" / "unseeded" / "music" / "yubal" / "existing_song.flac")
    existing_record = mocker.MagicMock(file_hash="old_hash", id="rec_123", file_path=expected_path)
